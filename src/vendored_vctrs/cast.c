...
r_obj* vec_cast_opts(const struct cast_opts* opts) {
  r_obj* x = opts->x;
  r_obj* to = opts->to;
  struct vendored_vctrs_arg* p_x_arg = opts->p_x_arg;
  struct vendored_vctrs_arg* p_to_arg = opts->p_to_arg;
...
r_obj* vec_cast_default_full(r_obj* x,
                             ...
                             struct vendored_vctrs_arg* p_to_arg,
                             struct r_lazy call,
                             enum s3_fallback s3_fallback,
                             bool from_dispatch) {
  r_obj* ffi_s3_fallback = KEEP(r_int(s3_fallback));

  r_obj* ffi_x_arg = KEEP(vendored_vctrs_arg(p_x_arg));
  r_obj* ffi_to_arg = KEEP(vendored_vctrs_arg(p_to_arg));
...
r_obj* vec_cast_dispatch_s3(const struct cast_opts* opts) {
  ...
  r_obj* r_x_arg = KEEP(vendored_vctrs_arg(opts->p_x_arg));
...
r_obj* vec_cast_common_params(r_obj* xs,
                              r_obj* to,
                              enum s3_fallback s3_fallback,
                              struct vendored_vctrs_arg* p_arg,
                              struct r_lazy call) {
