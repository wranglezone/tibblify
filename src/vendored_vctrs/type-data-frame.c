...
r_obj* df_list_unpack(r_obj* x) {
  ...
  for (; i < n_cols; ++i) {
    ...
    if (p_names[i] != vendored_strings_empty) {
      continue;
    }
...
r_obj* ffi_df_ptype2_opts(r_obj* x, r_obj* y, r_obj* opts, r_obj* frame) {
  ...
  struct vendored_vctrs_arg x_arg = new_lazy_arg(&x_arg_);

  struct r_lazy y_arg_ = { .x = syms.y_arg, .env = frame };
  struct vendored_vctrs_arg y_arg = new_lazy_arg(&y_arg_);
...
r_obj* df_ptype2_match(
  ...
  struct vendored_vctrs_arg* p_x_col_arg = new_subscript_arg(p_x_arg, x_names, x_len, &i);
  KEEP(p_x_col_arg->shelter);

  struct vendored_vctrs_arg* p_y_col_arg = new_subscript_arg(p_y_arg, y_names, y_len, &y_col_arg_loc);
...
r_obj* df_ptype2_loop(
  ...
  struct vendored_vctrs_arg* p_x_col_arg = new_subscript_arg_vec(p_x_arg, out, &i);
  KEEP(p_x_col_arg->shelter);

  struct vendored_vctrs_arg* p_y_col_arg = new_subscript_arg_vec(p_y_arg, out, &i);
  KEEP(p_y_col_arg->shelter);
...
r_obj* ffi_df_cast_opts(r_obj* x, r_obj* to, r_obj* opts, r_obj* frame) {
  ...
  struct vendored_vctrs_arg to_arg = new_lazy_arg(&to_arg_);
...
r_obj* df_cast_match(const struct cast_opts* opts,
  ...
  struct vendored_vctrs_arg* named_x_arg = new_subscript_arg(opts->p_x_arg, x_names, r_length(x_names), &x_arg_loc);
  KEEP(named_x_arg->shelter);

  struct vendored_vctrs_arg* named_to_arg = new_subscript_arg(opts->p_to_arg, to_names, to_len, &i);
  ...
  if (extra_len) {
    r_obj* ffi_x_arg = KEEP(vendored_vctrs_arg(opts->p_x_arg));
    r_obj* ffi_to_arg = KEEP(vendored_vctrs_arg(opts->p_to_arg));
...
r_obj* df_cast_loop(const struct cast_opts* opts, r_obj* names) {
  ...
  r_ssize i = 0;
  struct vendored_vctrs_arg* named_x_arg = new_subscript_arg(opts->p_x_arg, names, len, &i);
  KEEP(named_x_arg->shelter);

  struct vendored_vctrs_arg* named_to_arg = new_subscript_arg(opts->p_to_arg, names, len, &i);
  KEEP(named_to_arg->shelter);
