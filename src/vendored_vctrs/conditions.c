...
void stop_incompatible_type(SEXP x,
                            SEXP y,
                            struct vendored_vctrs_arg* x_arg,
  ...
  SEXP args[6] = {
    PROTECT(r_protect(x)),
    PROTECT(r_protect(y)),
    PROTECT(vendored_vctrs_arg(x_arg)),
    PROTECT(vendored_vctrs_arg(y_arg)),
...
void stop_incompatible_size(r_obj* x,
  ...
  r_obj* args[8] = {
    ...
    KEEP(r_int(x_size)),
    KEEP(r_int(y_size)),
    KEEP(vendored_vctrs_arg(x_arg)),
    KEEP(vendored_vctrs_arg(y_arg)),
...
void stop_incompatible_shape(SEXP x, SEXP y,
                             ...
                             struct vendored_vctrs_arg* p_x_arg, struct vendored_vctrs_arg* p_y_arg) {
  SEXP syms[8] = {
    r_sym("x"),
    r_sym("y"),
    r_sym("x_size"),
  ...
  SEXP args[8] = {
    ...
    PROTECT(r_int(axis)),
    PROTECT(vendored_vctrs_arg(p_x_arg)),
    PROTECT(vendored_vctrs_arg(p_y_arg)),
    NULL
  };
...
void stop_corrupt_factor_levels(SEXP x, struct vendored_vctrs_arg* arg) {
  SEXP call = PROTECT(Rf_lang3(Rf_install("stop_corrupt_factor_levels"),
                               PROTECT(r_protect(x)),
                               PROTECT(vendored_vctrs_arg(arg))));
  Rf_eval(call, vctrs_ns_env);
  never_reached("stop_corrupt_factor_levels");
}

void stop_corrupt_ordered_levels(SEXP x, struct vendored_vctrs_arg* arg) {
