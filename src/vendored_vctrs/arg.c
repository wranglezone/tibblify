...
/**
 * This takes a `struct vendored_vctrs_arg{}` linked list and calls the
 ...
 */
r_obj* vendored_vctrs_arg(struct vendored_vctrs_arg* arg) {
  if (!arg) {
    return chrs_empty;
  }
...
// vmax-protected
const char* vec_arg_format(struct vendored_vctrs_arg* p_arg) {
  r_obj* arg = KEEP(vendored_vctrs_arg(p_arg));
...
/**
 * Takes a `struct vendored_vctrs_arg{}` linked list and a buffer and calls the
 ...
 */
static
int fill_arg_buffer(struct vendored_vctrs_arg* arg,
                    char* buf,
                    r_ssize cur_size,
                    r_ssize tot_size) {
  if (arg->parent) {
...
struct vendored_vctrs_arg new_wrapper_arg(struct vendored_vctrs_arg* parent,
                                 const char* arg) {
  return (struct vendored_vctrs_arg) {
    .parent = parent,
...
struct vendored_vctrs_arg new_lazy_arg(struct r_lazy* arg) {
  return (struct vendored_vctrs_arg) {
    .parent = NULL,
    .fill = &lazy_arg_fill,
    .data = arg
...
struct subscript_arg_data {
  struct vendored_vctrs_arg self;
  r_obj* names;
  r_ssize n;
  r_ssize* p_i;
};

struct vendored_vctrs_arg* new_subscript_arg_vec(struct vendored_vctrs_arg* parent,
                                        r_obj* x,
                                        r_ssize* p_i) {
  r_obj* names = KEEP(vec_names(x));
  struct vendored_vctrs_arg* p_arg = new_subscript_arg(parent,
                                              names,
                                              vec_size(x),
                                              p_i);
  FREE(1);
...
struct vendored_vctrs_arg* new_subscript_arg(struct vendored_vctrs_arg* parent,
                                    r_obj* names,
                                    r_ssize n,
                                    r_ssize* p_i) {
  ...
  p_data->self = (struct vendored_vctrs_arg) {
    .shelter = shelter,
    .parent = parent,
  ...
  FREE(1);
  return (struct vendored_vctrs_arg*) p_data;
}
...
struct vendored_vctrs_arg new_counter_arg(struct vendored_vctrs_arg* parent,
...
struct arg_data_counter new_counter_arg_data(struct vendored_vctrs_arg* p_parent,
                                             r_ssize* i,
                                             r_obj** names) {
...
static
bool is_empty_arg(struct vendored_vctrs_arg* arg) {
  if (!arg) {
    return true;
  }
  char tmp[1];
