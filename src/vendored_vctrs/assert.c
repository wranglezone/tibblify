...
r_obj* ffi_obj_is_vector(r_obj* x) {
  ...
  return r_lgl(vendored_obj_is_vector(x, allow_null));
}
...
r_obj* ffi_obj_check_vector(r_obj* x, r_obj* frame) {
  ...
  struct vendored_vctrs_arg arg = new_lazy_arg(&arg_lazy);
...
bool list_all_vectors(r_obj* xs, enum vctrs_allow_null allow_null) {
  ...
  for (r_ssize i = 0; i < size; ++i) {
    r_obj* x = v_xs[i];
    if (!vendored_obj_is_vector(x, allow_null)) {
      return false;
    }
  }
...
void list_check_all_vectors(
  ...
  r_ssize i = 0;
  struct vendored_vctrs_arg* x_arg = new_subscript_arg_vec(p_xs_arg, xs, &i);
  KEEP(x_arg->shelter);
...
bool list_all_size(
  ...
  struct vendored_vctrs_arg* p_x_arg = new_subscript_arg(p_xs_arg, xs_names, xs_size, &i);
...
r_obj* ffi_check_list(r_obj* x, r_obj* frame) {
  ...
  struct vendored_vctrs_arg arg = new_lazy_arg(&arg_data);
