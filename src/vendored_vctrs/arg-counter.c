...
struct counters* new_counters(r_obj* names,
                              struct vendored_vctrs_arg* p_curr_arg,
                              struct vendored_vctrs_arg* p_parent_arg) {
  ...
  p_counters->next_arg = (struct vendored_vctrs_arg*) &p_counters->next_counter;
...
void counters_shift(struct counters* p_counters) {
  ...
  SWAP(struct vendored_vctrs_arg, p_counters->curr_counter, p_counters->next_counter);
  SWAP(r_ssize*, p_counters->curr_counter_arg_data.i, p_counters->next_counter_arg_data.i);

  // Update the handles to `vendored_vctrs_arg`
  p_counters->curr_arg = (struct vendored_vctrs_arg*) &p_counters->curr_counter;
  p_counters->next_arg = (struct vendored_vctrs_arg*) &p_counters->next_counter;
...
r_obj* reduce(
  r_obj* current,
  struct vendored_vctrs_arg* p_current_arg,
  struct vendored_vctrs_arg* p_parent_arg,
  r_obj* rest,
