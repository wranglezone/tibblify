...
struct vendored_vctrs_arg args_needles;
struct vendored_vctrs_arg args_haystack;
...
SEXP vctrs_match(SEXP needles, SEXP haystack, SEXP na_equal,
  ...
  struct vendored_vctrs_arg needles_arg = new_lazy_arg(&needles_arg_);

  struct r_lazy haystack_arg_ = { .x = syms.haystack_arg, .env = frame };
  struct vendored_vctrs_arg haystack_arg = new_lazy_arg(&haystack_arg_);
