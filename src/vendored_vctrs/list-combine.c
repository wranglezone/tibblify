...
r_obj* ffi_list_combine(
  ...
  struct vendored_vctrs_arg xs_arg = new_lazy_arg(&xs_arg_lazy);

  struct r_lazy indices_arg_lazy = { .x = syms.indices_arg, .env = ffi_frame };
  struct vendored_vctrs_arg indices_arg = new_lazy_arg(&indices_arg_lazy);

  struct r_lazy default_arg_lazy = { .x = syms.default_arg, .env = ffi_frame };
  struct vendored_vctrs_arg default_arg = new_lazy_arg(&default_arg_lazy);
...
r_obj* list_combine(
  ...
  struct vendored_vctrs_arg* p_xs_arg,
  struct vendored_vctrs_arg* p_indices_arg,
  struct vendored_vctrs_arg* p_default_arg,
...
r_obj* list_combine_for_list_unchop(
  ...
  struct vendored_vctrs_arg* p_indices_arg = vec_args.indices;
  ...
  struct vendored_vctrs_arg* p_default_arg = vec_args.empty;
...
r_obj* list_combine_impl(
 ...
 const struct vec_proxy_assign_opts proxy_assign_opts = {
  ...
  struct vendored_vctrs_arg* p_x_arg = new_subscript_arg(
    p_xs_arg,
    xs_names,
    xs_size,
    &xs_i
...
void df_list_combine_common_class_fallback(
  ...
  for (r_ssize i = 0; i < n_cols; ++i) {
    ...
    if (is_data_frame(col) && needs_df_list_combine_common_class_fallback(ptype_col)) {
      ...
      struct vendored_vctrs_arg* p_xs_col_arg = vec_args.empty;
      struct vendored_vctrs_arg* p_default_col_arg = vec_args.empty;
...
bool needs_list_combine_homogeneous_fallback(
  ...
  if (!vendored_obj_is_vector(first, VCTRS_ALLOW_NULL_no)) {
    return false;
  }
