...
- [`tibblify()`](https://tibblify.wrangle.zone/dev/reference/tibblify.md)
  and `.fill` arguments now accept values that can be losslessly coerced
  to character or factor targets, such as `1L` when a character field is
  expected, or `"a"` when a factor field is expected (using stbl’s C API
  for `chr` and `fct` targets)
  ([@jonthegeek](https://github.com/jonthegeek),
  [\#340](https://github.com/wranglezone/tibblify/issues/340)).
