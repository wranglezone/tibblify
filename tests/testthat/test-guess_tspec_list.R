...
test_that("guess_tspec_list can guess spec for discog", {
  expect_snapshot(guess_tspec_list(discog) |> print())
})

test_that("guess_tspec_list can guess spec for gh_users", {
  expect_snapshot(guess_tspec_list(gh_users) |> print())
})

test_that("guess_tspec_list respects empty_list_unspecified (#83)", {
  spec <- guess_tspec_list(got_chars)
  ...
  spec2 <- guess_tspec_list(got_chars, empty_list_unspecified = TRUE)
...
test_that("guess_tspec_list can guess spec for gsoc-2018", {
  x <- read_sample_json("gsoc-2018.json")
  expect_snapshot(guess_tspec_list(x))
})

test_that("guess_tspec_list can guess spec for twitter", {
