...
test_that("colmajor: tib_vector works", {
  ...
  # can parse
  expect_equal(
    tib_cm(tib_lgl_vec("x"), x = list(c(TRUE, FALSE))),
    tibble(x = vctrs::list_of(c(TRUE, FALSE)))
  )
  expect_equal(
    tib_cm(tib_vector("x", dtt), x = list(c(dtt, dtt + 1))),
    tibble(x = vctrs::list_of(c(dtt, dtt + 1)))
  )
...
test_that("colmajor: tib_row works", {
  ...
  expect_equal(
    ...
    tibble(
      x = tibble(
        row = tibble(int = 1:2, chr_vec = vctrs::list_of("a", c("b", "c")))
      )
    )
  )
})
...
test_that("colmajor: tib_df works", {
  ...
  expect_equal(
    ...
    tibble(x = vctrs::list_of(tibble(int = 1:2), tibble(int = integer())))
  )
  ...
  expect_equal(
    tib_cm(
      tib_df(
        "x",
        tib_row("row", tib_int("int")),
        tib_df("df", tib_int("df_int"))
      ),
      x = list(
        list(
          row = list(int = 1:2),
          df = list(list(df_int = 1), list(df_int = 3:4))
        ),
    ...
    tibble(
      x = vctrs::list_of(
        tibble(
          row = tibble(int = 1:2),
          df = vctrs::list_of(tibble(df_int = 1), tibble(df_int = 3:4))
        ),
        tibble(
          row = tibble(int = 3),
          df = vctrs::list_of(NULL, .ptype = tibble(df_int = integer()))
        )
      )
    )
  )
...
test_that("colmajor: tibble with list columns work - #43", {
  ...
  y <- tibble::tibble(x = list(tibble(a = 1:2), NULL, tibble(a = 1)))
  spec <- tspec_df(
    x = tib_df("x", tib_dbl("a"), .required = FALSE),
    .input_form = "colmajor"
  )
  expect_equal(
    tibblify(y, tspec_df(x = tib_df("x", tib_dbl("a")))),
...
test_that("colmajor: can calculate size", {
  ...
  expect_equal(
    tibblify(
      list(x = 1:2),
      tspec_df(tib_int("x"), .input_form = "colmajor")
    ),
    tibble(x = 1:2)
  )
  ...
  expect_equal(
    tibblify(
      list(x = c(x_rcrd, x_rcrd + 1)),
      tspec_df(tib_scalar("x", x_rcrd), .input_form = "colmajor")
    ),
    tibble(x = c(x_rcrd, x_rcrd + 1))
  )
  ...
  expect_equal(
    tibblify(
      list(x = list(1:4, 1:3)),
      tspec_df(tib_int_vec("x"), .input_form = "colmajor")
    ),
    tibble(x = vctrs::list_of(1:4, 1:3))
  )
  ...
  expect_equal(
    tibblify(
      list(x = list(a = 1:2)),
      tspec_df(tib_row("x", tib_int("a")), .input_form = "colmajor")
    ),
    tibble(x = tibble(a = 1:2))
  )
  ...
  expect_equal(
    tibblify(
      ...
      tspec_df(tib_df("x", tib_int("a")), .input_form = "colmajor")
    ),
    tibble(x = vctrs::list_of(tibble(a = 1:2), tibble(a = 1)))
  )
